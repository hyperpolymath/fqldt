...
macro_rules
  | `(tactic| formdb_bounds) => `(tactic| first | omega | norm_num | decide)

-- Auto-solve provenance proofs
syntax "formdb_prov" : tactic
macro_rules
  | `(tactic| formdb_prov) => `(tactic| 
      simp only [Tracked, NonEmptyString]; 
      constructor <;> decide)

-- Auto-solve PROMPT score proofs
syntax "formdb_prompt" : tactic
macro_rules
  | `(tactic| formdb_prompt) => `(tactic|
...
WITH_PROOF {
  ...
  provenance_exists: by formdb_prov,
  confidence_in_range: by formdb_bounds
}
```
...
WITH_PROOF {
  all_scores_in_bounds: by formdb_prompt,
  overall_computed_correctly: by formdb_prompt,
  rationale_nonempty: by formdb_prov
};
...
ADDED_BY "journalist_alice"
WITH_PROOF {
  weight_in_bounds: by formdb_bounds,
  reasoning_nonempty: by formdb_prov
};
...
WITH_PROOF {
  ...
  new_confidence_valid: by formdb_bounds,
  correction_reason_nonempty: by formdb_prov
};
...
WITH_PROOF {
  confidence_in_bounds: by formdb_bounds
};
