...
//
// fdb_persist.zig - FormDB persistent storage backend (minimal)
//
...
var debug_init_counter: i32 = 0; // Debug: incremented each time fdb_init runs
...
pub fn fdb_init(path_ptr: ?[*]const u8, path_len: usize) i32 {
...
pub fn fdb_debug_init_counter(dummy: i32) i32 {
    _ = dummy;
...
pub fn fdb_test_fresh(input: i32) i32 {
    test_counter += input;
    return test_counter;
}

/// Close database
pub fn fdb_close() FdbStatus {
...
pub fn fdb_table_count(table_ptr: ?[*]const u8, table_len: usize) u64 {
...
pub fn fdb_save(dummy: i32) FdbStatus {
    _ = dummy;
    if (!initialized) return .generic_error;
    dirty = false;
    return .ok;
...
pub fn fdb_is_init(dummy: i32) bool {
    _ = dummy;
    return initialized;
}

/// Debug: force set initialized for testing
pub fn fdb_debug_set_init(val: bool) void {
    initialized = val;
}

/// Debug: get initialized as integer (for debugging Lean FFI)
pub fn fdb_debug_init_status() i32 {
...
test "basic persistence" {
    try std.testing.expectEqual(FdbStatus.ok, fdb_init(null, 0));
    try std.testing.expect(fdb_is_init());

    var row_id: u64 = 0;
    try std.testing.expectEqual(FdbStatus.ok, fdb_insert_row(
        "test",
        4,
        null,
        0,
    ...
    try std.testing.expectEqual(@as(u64, 1), fdb_table_count("test", 4));

    try std.testing.expectEqual(FdbStatus.ok, fdb_save());
    try std.testing.expectEqual(FdbStatus.ok, fdb_close());
    try std.testing.expect(!fdb_is_init());
}
